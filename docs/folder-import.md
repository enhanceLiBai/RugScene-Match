...
`backend.merge_style_variants` 仅合并同一商品 ID 下、末尾为已确认营销后缀的目录：`可享国补`、`国补`、`预售15天`、`百搭不出错`、`轻奢风推荐`。它只在无后缀基础款目录已存在时移动图片；没有基础款目录时保留原样，避免猜测。材质、颜色、花型等其他括号内容不会处理。
...
开始入库：在命令后增加 `--execute`，可加 `--limit 20` 限制本轮扫描文件数。移除 limit 处理全部。每张图先校验格式及 20 MiB 上限，再调用 DeepSeek JSON 模式，深度思考关闭。脚本不再做图片合格筛选：格式有效的图片均会生成场景、沙发、地板和画面色调标签后写入图片、向量、标签和商品关联。原目录不修改；API、格式或标签响应错误记为 failed，重复运行自动重试。

处理报告默认 data/folder-import.jsonl；包含源路径、哈希、商品、款式、标签、图片ID和状态。保留报告，重复运行跳过已成功的同内容同款式记录；不是新增数据库恢复机制，数据库被人工修改后应核对报告再运行。不同内容图片不会因为文件名相同被跳过。文件异常只记录错误类型，不输出密钥或响应原文。
