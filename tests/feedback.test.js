...
test('管理页展示保存的客户照片与全部推荐，图片缺失提供说明', () => {
  ...
  context.renderFeedbackHistory(container, {
    ...
    payload: {query_scene: {room: '客厅'}, results: [
      {rank: 1, product_id: 'A', style: ' 云纹米色 ', similarity: 95, matched_buyer_image_url: '/api/images/1', product_image_url: '/api/images/2'},
  ...
  assert.deepEqual(all.filter(n => n.textContent?.startsWith('款式：')).map(n => n.textContent), ['款式：云纹米色']);
