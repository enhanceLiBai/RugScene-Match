...
def test_brown_color_group_matches():
    query = {'room':'客厅','sofa_status':'present','sofa_color':'卡其色',
             'floor_status':'present','floor_color':'浅灰色','floor_material':'瓷砖/石材'}
    candidate = {**query, 'sofa_color':'浅棕色'}
    assert score_scene(query, candidate, 80)[0] == 80
